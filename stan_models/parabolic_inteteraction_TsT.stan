// Stan parabolic model with 5 predictors and an interaction between Temperature seasonality and Temperature
data {
...
transformed data { // this will define interactions included in the model
 vector[N] inter2 = x3 .* x1; //the '.' makes it work like R 
 vector[N] inter3 = x3 .* x4; //the '.' makes it work like R 
}
...
model {
 y ~ normal(alpha + x1 * beta1 + square(x1) * beta2 + x2 * beta3 + square(x2) * beta4 +
                          ...
                          x5 * beta9 + square(x5) * beta10 + inter2*beta11 + square(inter2)*beta12 +  
                          inter3*beta13 + square(inter3)*beta14 +
